...
jQuery(document).ready(function ($) {
	...
	function headerSection(section, sectionButton) {
		...
		if (headerSectionAnimation == 'complete' && currentHeaderSection != sectionButton) {
			...
			if (currentHeaderSection == 'shoppingCart' && sectionButton == 'mainMenu') {
				...
				tempDelay = 600;
				$('.shoppingCartWrapper').stop(true, true).animate({
					'margin-top': -shoppingCartHeight
				...
				tempDelay = 600;
				$('.mainMenuOuterWrapper').stop(true, true).animate({
			...
			$(section).stop(true, true).delay(tempDelay).animate({
				'margin-top': 0
			}, 600, 'easeInQuart', function () {
				headerSectionAnimation = 'complete';
				currentHeaderSection = sectionButton;
			});
		...
		if (filterVal != 'all') {
			...
			$('.portfolioFilterableItemWrapper').each(function () {
				...
				if ($.inArray(filterVal, itemCategories) > -1) {
					...
					$('.filteredPortfolioItem').stop(true, true).animate({
						opacity: 1
					}, 300, 'easeOutCubic');
			...
			$('.portfolioFilterableItemWrapper').stop(true, true).animate({
				opacity: 1
			}, 300, 'easeOutCubic');
		...
		if (!($(this).parent().hasClass('drop-downActive'))) {
			...
			$('.drop-downWrapper').each(function () {
				...
				if ($(this).hasClass('drop-downActive')) {
					...
					$(this).find('> .drop-downItemsWrapper').stop(true, true).animate({
						height: 'hide'
					}, 300, 'easeOutCubic');
				};
			...
			$(this).parent().find('> .drop-downItemsWrapper').stop(true, true).animate({
				height: 'show'
			}, 300, 'easeOutCubic');
			...
			$(this).parent().find('> .drop-downItemsWrapper').stop(true, true).animate({
				...
				$(this).parent().removeClass('drop-downActive');
			});
	...
	$('.drop-downItem').click(function () {
		...
		$(this).parent().parent().find('> .drop-downItemsWrapper').stop(true, true).animate({
			height: 'hide'
		}, 300, 'easeOutCubic');
	...
	$('.accordionButton').click(function (e) {
		...
		if ($(this).hasClass('currentAccordion')) {
			...
			$(this).parent().find('.accordionContentWrapper').stop(true, true).animate({
				height: 'hide'
			}, 300, 'easeOutCubic', function () {
				$(this).parent().find('.accordionButton').removeClass('currentAccordion');
			});
			...
			$(this).parent().find('.accordionContentWrapper').stop(true, true).animate({
				...
				$(this).parent().find('.accordionButton').addClass('currentAccordion');
			});
